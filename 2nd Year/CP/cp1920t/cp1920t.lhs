...
%format Cp.ap = "\mathsf{ap}"
%format Probability.cond = "\mathsf{cond}"
...
\textbf{Grupo} nr. & 47
\\\hline
a89447 & Sara Alexandra Gomes Marques	
\\
...
tar = cataExp g 
     where
       g = either (singl . split nil id) (uncurry fstconcat)
       fstconcat x y = [(a,b) | l <- y, (fst,b) <- l, let a = x ++ fst]

dic_rd p t = lookup p (dic_exp t) 

dic_in p s = Term "" . map (bmap  id singl) . untar . (curry (cons) (p,s)) . discollect . dic_exp 
...
\begin{code}
maybe_id k x = maybe k id x
node_maior k x = p2p ((k >= Just x), True) (x==Nothing)
node_menor k x = p2p ((k <= Just x), True) (x==Nothing)

maisDir = cataBTree g
  where g = either (const Nothing) ( Just . uncurry (maybe_id) . (id >< p2) )

maisEsq = cataBTree g
  where g = either (const Nothing) ( Just . uncurry (maybe_id) . (id >< p1) )

isOrd = p1 . isOrd'

insOrd x = p1 . insOrd' x

maxBtree :: (Ord a ) =>  a -> BTree a -> Bool
maxBtree a = cataBTree g
        where g = either (const True) (res)
              res (x,(b,c)) = (x<a) && b && c 

minBtree :: (Ord a ) => a-> BTree a -> Bool
minBtree a = cataBTree g
        where g = either (const True) (res)
              res (x,(b,c)) = (x>a) && b && c 

isOrd' = cataBTree g
       where g = either (const (True,Empty)) sndf
             sndf (a,((b,c),(d,e))) = ((maxBtree a c) && (minBtree a e) && b && d, Node (a,(c,e)) ) 

insOrd' x = cataBTree g
       where g = either ((const ((Node (x,(Empty,Empty))),Empty))) (sndf)
             sndf (a,((b,c),(d,e))) = if (x > a) then (Node (a,(b,e)),Node (a,(c,e))) else (Node (a,(d,c)),Node (a,(c,e)))

rrot (Node (x,(Node (y,(a,b)),right))) = Node (y,(a,Node (x,(b,right))))

lrot (Node(x,(left,(Node (y,(a,b))))) ) = Node (y,((Node (x,(left,a)),b)))  

splay l t = (cataList (either (const id) (parseBTree . either ((const rrot) >< id) ((const lrot) >< id) . grd (id . p1) )) l) t
          where parseBTree (f,g) =  either (const Empty) (f . g . Node) .  outBTree
...
navLTree = cataLTree g 
  where g = either (constLeaf) (parseList)
        constLeaf x = const (Leaf x)
        parseList (a,b)  =  either (Fork . ( split ( a . nil) (b . nil))) ((either (a . p2) (b. p2)) . grd p1) . outList
...
bnavLTree = cataLTree g
  where g = either (constLeaf) (parseBTree)
        constLeaf x = const (Leaf x)
        parseBTree (a,b)  =  either (Fork . ( split ( a . const Empty) (b . const Empty))) ((either (a . p1 . p2 ) (b . p2 . p2)) . grd p1) . outBTree


pbnavLTree = cataLTree g
    where g = either (constLeaf) (parseBTreeDist)
          constLeaf x = const (return (Leaf x))
          parseBTreeDist (a,b)  =  either (return . Fork . split (coU . a) (coU . b) . const Empty ) (probCond (a,b)) . outBTree
          probCond (a,b) (dist,x) = uncurry (Probability.cond dist) ((a >< b) x) 
          coU = head . extract 
...
janela = InWindow
             "Truchet-Smith Mosaic"  -- window title
             (800, 800)              -- window size
             (0, 0)                  -- window position


truchetMosaic sx sy = let draw x y [] = []
                          draw x y (t:ts) | x+400 < (sx*80)   = put ((fromIntegral x),y) t : draw (x+80) y ts 
                                          | otherwise        = draw (-400) (y+80) (t:ts)
                      in do 
                        nt1 <- randomRIO (0,(sx*sy))
                        list <- permuta ( (replicate nt1 truchet1) ++ (replicate ((sx*sy)-nt1) truchet2) )
                        display janela white (pictures (draw (-400) (-400) list))

main = do truchetMosaic 20 20
...
\end{code}
    \begin{figure}[htp]
    \centering
    \includegraphics[scale=0.30]{images/truchet_entregar.png}
    \caption{Um mosaico 10|><|10 gerado pelo progama}
    \label{fig:truchet_solucao}
    \end{figure}
